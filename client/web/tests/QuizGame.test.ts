...
describe('QuizGame', () => {
  ...
  it('updates score when correct answer is selected', async () => {
    ...
    const correctAnswer = wrapper.findAll('.quiz-button')
  ...
  it('ends game when incorrect answer is selected', async () => {
    ...
    const incorrectAnswer = wrapper.findAll('.quiz-button')
  ...
  it('awards correct points based on question difficulty', async () => {
    ...
    await new Promise(r => setTimeout(r, 2000));
    const secondCorrectAnswer = wrapper.findAll('.quiz-button')
...
export const getNextQuestion = (previousQuestion: Question): Question => {
  const filteredQuestions = questionBank.value.filter(q => q.id !== previousQuestion.id);
