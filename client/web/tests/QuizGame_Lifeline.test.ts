...
import QuizGame from '../src/components/QuizGame.vue'
import LifelineButton from '../src/components/LifelineButton.vue';
...
describe('QuizGame.vue', () => {
  it('should use lifeline and hide two incorrect answers', async () => {
    ...
    const incorrectAnswers = initialAnswers.filter(answer => !answer.isCorrect);
    ...
    const hiddenAnswers = wrapper.vm.currentQuestion.answers.filter(answer => answer.isHidden);
    expect(hiddenAnswers.length).toBe(2);

    // Check that the correct answer is not hidden
    const correctAnswer = wrapper.vm.currentQuestion.answers.find(answer => answer.isCorrect);
    expect(correctAnswer.isHidden).toBe(false);
  });

  it('should not allow using lifeline if canUseLifeline is false', async () => {
    const wrapper = mount(QuizGame);
    wrapper.vm.canUseLifeline = false;
